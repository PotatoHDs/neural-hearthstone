        ...
        for i in range(times):
            ...
            for target in targets:
                ...
                if isinstance(source, Player):
                    source.add_targetedaction_log(
                        {'class': self, 'source': source, 'target': target})  ## log for action
                elif isinstance(source, PlayableCard):
                    source.controller.add_targetedaction_log(
                        {'class': self, 'source': source, 'target': target})  ## log for action
...
class Destroy(TargetedAction):
    ...
    def do(self, source, target):
        if not target:
            return
        if target is None:
            log.info("%r marks nothing for imminent death", source)
...
class SidequestManaCounter(TargetedAction):
    ...
    def do(self, source, target, card, amount, targetaction):
        log.info("Setting Counter on %r is added by %d->%d, %r", target, card.cost,
                 (target._sidequest_counter_ + card.cost), targetaction)
...
class SidequestLostInTheParkCounter(TargetedAction):  ##  SW_428 Lost in the park
    ...
    def do(self, source, target, amount, targetaction):
        log.info("Setting Counter on %r rolls by %d->%d, %r", target, target.controller.lost_in_the_park,
                 (target._sidequest_counter_ + target.controller.lost_in_the_park), targetaction)
...
class SummonAdventurerWithBonus(TargetedAction):
    ...
    def do(self, source, target):
        new_minion = random.choice(
            ['WC_034t', 'WC_034t2', 'WC_034t3', 'WC_034t4', 'WC_034t5', 'WC_034t6', 'WC_034t7', 'WC_034t7', ])
